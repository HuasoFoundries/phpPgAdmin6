                    ...
                    if (is_array($_REQUEST['table'])) {
                        ...
                        foreach ($_REQUEST['table'] as $o) {
                            ...
                            if ($status == 0) {
                                ...
                                $this->doDefault($type,
                                    sprintf('%s%s: %s<br />', $msg, htmlentities($o, ENT_QUOTES, 'UTF-8'), $lang['strclusteredbad']));
        ...
        public function doAdmin($type, $msg = '')
        {
            ...
            if ($data->hasAutovacuum()) {
                ...
                $columns = [
                    ...
                    'autovacuum_vacuum_cost_delay'    => [
                        'title' => $lang['strvacuumcostdelay'],
                        'field' => Decorator::concat(Decorator::callback('enlight',
                            ['autovacuum_vacuum_cost_delay', $defaults['autovacuum_vacuum_cost_delay']]), 'ms'),
                        'type'  => 'verbatim',
                    ],
                    'autovacuum_vacuum_cost_limit'    => [
                        'title' => $lang['strvacuumcostlimit'],
        ...
        public function doVacuum($type, $confirm = false)
        {
            ...
            if ($confirm) {
                ...
                if (is_array($_REQUEST['table'])) {
                    ...
                    //we must pass table here. When empty, vacuum the whole db
                    $status = $data->vacuumDB($_REQUEST['table'], isset($_REQUEST['vacuum_analyze']), isset($_REQUEST['vacuum_full']),
                        isset($_REQUEST['vacuum_freeze']));
                    if ($status == 0) {
                        $misc->setReloadBrowser(true);
                        $this->doAdmin($type, $lang['strvacuumgood']);
