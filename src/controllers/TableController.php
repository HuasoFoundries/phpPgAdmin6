        ...
        public function doInsertRow($confirm, $msg = '')
        {
            ...
            if ($confirm) {
                ...
                if ($attrs->recordCount() > 0) {
                    ...
                    while (!$attrs->EOF) {
                        ...
                        if (($fksprops !== false) && isset($fksprops['byfield'][$attrs->fields['attnum']])) {
                            echo $data->printField("values[{$attrs->fields['attnum']}]", $_REQUEST['values'][$attrs->fields['attnum']], 'fktype'
                                /*force FK*/,
                                [
                ...
                if ($_SESSION['counter']++ == $_POST['protection_counter']) {
                    $status =
                        $data->insertRow($_POST['table'], $_POST['fields'], $_POST['values'], $_POST['nulls'], $_POST['format'], $_POST['types']);
                    if ($status == 0) {
                    ...
                    if ($status == 0) {
                        foreach ($_REQUEST['table'] as $t) {
                            ...
                            if ($status == 0) {
                                ...
                                return $this->doDefault(sprintf('%s%s: %s<br />', $msg, htmlentities($t, ENT_QUOTES, 'UTF-8'),
