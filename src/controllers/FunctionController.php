    ...
    class FunctionController extends BaseController
    {
        ...
        public function doSaveCreate()
        {
            ...
            if ($_POST['formFunction'] == '') {
                ...
                $status =
                    $data->createFunction($_POST['formFunction'], empty($_POST['nojs']) ? buildFunctionArguments($_POST) : $_POST['formArguments'],
                    ...
                    if ($status == 0) {
                        foreach ($_POST['function_oid'] as $k => $s) {
                            ...
                            if ($status == 0) {
                                ...
                                $data->endTransaction();
                                $this->doDefault(sprintf('%s%s: %s<br />', $msg, htmlentities($_POST['function'][$k], ENT_QUOTES, 'UTF-8'),
        ...
        public function buildFunctionArguments($arrayVars)
        {
            if (isset($_POST['formArgName'])) {
                ...
                foreach ($arrayVars['formArgName'] as $pK => $pV) {
                    $arrayArgs[] =
                        $arrayVars['formArgModes'][$pK] . ' ' . trim($pV) . ' ' . trim($arrayVars['formArgType'][$pK]) . $arrayVars['formArgArray'][$pK];
                }
